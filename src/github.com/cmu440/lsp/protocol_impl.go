				...
				if msg, rAddr, err := l.recvMsg(c); err != nil {
					...
					if strings.EqualFold(rAddr.String(), c.rAddr.String()) {
						if msg.Type == MsgAck && msg.SeqNum == 0 {
                            ...
                            go l.recvMsgLoop(c)
                            go l.handleRecvMsgLoop(c)
                            go l.handleSendMsgLoop(c)
							return c, nil
						}
					}
				}
...
func (l *LSP) handleRecvMsg(c *LSPConn, msg *Message) {
    ...
    if msg.Type == MsgData {
        ...
        if i == msg.SeqNum+1 {
            // 同上
            for j := i; ; j++ {
                if _, ok := c.sendWindow.Get(i).(*AckMsg); !ok {
                    break
                }
                counter++
            }
...
// 核心函数，处理消息
func (l *LSP) handleRecvMsgLoop(c *LSPConn) {
    // fmt.Println(connID)
    for {
        select {
        case msg := <- c.handleRecvMsgChan:
            // fmt.Println(msg)
            if msg.Type == MsgData {
                for msg.SeqNum > c.recvWindowMax.Value() {
                    // 阻塞等待
                    select {
                    default:
                        continue
                ...
                // fmt.Println(connID, l.connMap[connID])
                l.handleRecvMsg(c, msg)
            }
